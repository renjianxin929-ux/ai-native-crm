/**
 * Battle Card Backend V1 — Schema（幂等 ensure）。
 * 与 src-tauri/migrations/005_customer_battle_card.sql 保持同一契约；
 * 运行时迁移由 ensureBattleCardSchema 驱动（与现有 ensure* 惯例一致）。
 */

import type { DatabaseLike } from '../db';

export const BATTLE_CARD_SCHEMA_VERSION = 'battle-card-payload-v1';
export const BATTLE_CARD_PARSER_VERSION = 'battle-card-parser-v1';

export const BATTLE_CARD_TABLE_SQL: readonly string[] = [
...
export const BATTLE_CARD_INDEX_SQL: readonly string[] = [
...
/** customers 主表最小指针字段（仅缺失时补充；已有等价字段则不重复添加）。 */
export const BATTLE_CARD_CUSTOMER_POINTER_COLUMNS: readonly { readonly name: string; readonly ddl: string }[] = [
  { name: 'current_stage_card_id', ddl: 'ALTER TABLE customers ADD COLUMN current_stage_card_id TEXT' },
  { name: 'battle_card_status', ddl: "ALTER TABLE customers ADD COLUMN battle_card_status TEXT NOT NULL DEFAULT 'NONE'" },
  { name: 'last_battle_review_at', ddl: 'ALTER TABLE customers ADD COLUMN last_battle_review_at TEXT' },
];

export async function ensureBattleCardSchema(db: DatabaseLike): Promise<void> {
  for (const sql of BATTLE_CARD_TABLE_SQL) {
    await db.execute(sql);
  }
  for (const sql of BATTLE_CARD_INDEX_SQL) {
    await db.execute(sql);
  }
  await ensureBattleCardCustomerPointers(db);
}

export async function ensureBattleCardCustomerPointers(db: DatabaseLike): Promise<void> {
  const columns = await db.select<{ name?: string }>('PRAGMA table_info(customers)');
  for (const column of BATTLE_CARD_CUSTOMER_POINTER_COLUMNS) {
