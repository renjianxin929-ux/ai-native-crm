export interface CustomerMemoryDatabase {
  execute(sql: string, bindings?: unknown[]): Promise<{ rowsAffected: number }>;
}

export const CUSTOMER_MEMORY_SCHEMA_SQL = [
...
export async function ensureCustomerMemorySchema(db: CustomerMemoryDatabase): Promise<void> {
  for (const sql of CUSTOMER_MEMORY_SCHEMA_SQL) await db.execute(sql);
}
