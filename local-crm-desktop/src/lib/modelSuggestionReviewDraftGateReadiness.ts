import type {
  AdaptedModelSuggestionBoundaryCandidate,
  ModelSuggestionAdapterBoundaryResult,
} from './modelSuggestionAdapterBoundaryReadiness';

export const MODEL_SUGGESTION_REVIEW_DRAFT_GATE_VERSION = 'v1';

type BoolFalse = false;
type BoolTrue = true;

const FALSE_VALUE = false;
const TRUE_VALUE = true;

export type ModelSuggestionReviewDraftGateBlockedReason =
  | 'invalid_request_kind'
  | 'illegal_enqueue_allowed'
  | 'illegal_review_queue_entry_allowed'
  | 'illegal_confirmed_action_allowed'
  | 'illegal_confirmed_action_envelope_allowed'
  | 'illegal_human_confirmation_allowed'
  | 'illegal_runner_allowed'
  | 'illegal_execution_allowed'
  | 'illegal_write_plan_entry_allowed'
  | 'illegal_db_allowed'
  | 'illegal_network_allowed'
  | 'illegal_env_read_allowed'
  | 'illegal_model_call_allowed'
  | 'invalid_source_result_kind'
  | 'source_answer_missing'
  | 'source_adapter_boundary_blocked'
  | 'source_boundary_candidates_missing'
  | 'source_boundary_candidates_empty'
  | 'illegal_source_called_real_provider'
  | 'illegal_source_used_network'
  | 'illegal_source_reads_database'
  | 'illegal_source_writes_database'
  | 'illegal_source_entered_review_queue'
  | 'illegal_source_entered_human_confirmation'
  | 'illegal_source_entered_write_plan'
  | 'illegal_source_produced_confirmed_action'
  | 'illegal_source_produced_executable_proposal'
  | 'illegal_source_candidate_executable'
  | 'illegal_source_candidate_confirmed_action'
  | 'illegal_source_candidate_human_confirmed'
  | 'illegal_source_candidate_emits_confirmed_action_envelope'
  | 'illegal_source_candidate_enters_review_queue'
  | 'illegal_source_candidate_enters_write_plan'
  | 'illegal_source_candidate_status'
  | 'illegal_output_text_propagated'
  | 'illegal_draft_candidate_enqueued'
  | 'illegal_draft_candidate_executable'
  | 'illegal_draft_candidate_confirmed_action'
  | 'illegal_draft_candidate_confirmed_action_envelope'
  | 'illegal_draft_candidate_emits_confirmed_action_envelope'
  | 'illegal_draft_candidate_emits_review_queue_candidate'
  | 'illegal_draft_candidate_enters_review_queue'
  | 'illegal_draft_candidate_enters_write_plan';

export type ModelSuggestionReviewDraftStatus =
  | 'draft_requires_human_review'
  | 'draft_blocked_source'
  | 'draft_policy_only';

export interface ModelSuggestionReviewDraftGateRequest {
  kind: 'MODEL_SUGGESTION_REVIEW_DRAFT_GATE_REQUEST';
  version: typeof MODEL_SUGGESTION_REVIEW_DRAFT_GATE_VERSION;
  request_id: string;
  source_adapter_boundary_result: ModelSuggestionAdapterBoundaryResult;
  review_draft_gate_only: BoolTrue;
  caller_provided_only: BoolTrue;
  allow_enqueue: BoolFalse;
  allow_review_queue_entry: BoolFalse;
  allow_confirmed_action: BoolFalse;
  allow_confirmed_action_envelope: BoolFalse;
  allow_human_confirmation: BoolFalse;
  allow_runner: BoolFalse;
  allow_execution: BoolFalse;
  allow_write_plan_entry: BoolFalse;
  allow_db: BoolFalse;
  allow_network: BoolFalse;
  allow_env_read: BoolFalse;
  allow_model_call: BoolFalse;
}

export interface ModelSuggestionReviewDraftGatePlan {
  kind: 'MODEL_SUGGESTION_REVIEW_DRAFT_GATE_PLAN';
  version: typeof MODEL_SUGGESTION_REVIEW_DRAFT_GATE_VERSION;
  executable: BoolFalse;
  persisted: BoolFalse;
  reason: 'model_suggestion_review_draft_gate_readiness_only';
  request: ModelSuggestionReviewDraftGateRequest;
  allowed_operations: readonly [
    'validate_caller_provided_adapter_boundary_result',
    'project_safe_boundary_candidates_to_review_draft',
    'build_review_draft_gate_result',
  ];
  forbidden_operations: readonly string[];
}

export interface ModelSuggestionReviewDraftCandidate {
  kind: 'MODEL_SUGGESTION_REVIEW_DRAFT_CANDIDATE';
  version: typeof MODEL_SUGGESTION_REVIEW_DRAFT_GATE_VERSION;
  review_draft_id: string;
  source_boundary_candidate_id: string;
  source_suggestion_candidate_id: string;
  source_output_id: string;
  review_draft_status: ModelSuggestionReviewDraftStatus;
  priority_band: 'high' | 'medium' | 'low';
  title: string;
  summary: string;
  evidence_summary: string;
  risk_summary: string;
  review_draft_only: BoolTrue;
  suggestion_only: BoolTrue;
  dry_run_only: BoolTrue;
  requires_human_review: BoolTrue;
  enqueued: BoolFalse;
  executable: BoolFalse;
  confirmed_action: BoolFalse;
  confirmed_action_envelope: BoolFalse;
  human_confirmed: BoolFalse;
  approval_recorded: BoolFalse;
  enters_review_queue: BoolFalse;
  enters_human_confirmation: BoolFalse;
  enters_write_plan: BoolFalse;
  emits_confirmed_action_envelope: BoolFalse;
  emits_review_queue_candidate: BoolFalse;
  produces_confirmed_action: BoolFalse;
  produces_executable_proposal: BoolFalse;
  reads_database: BoolFalse;
  writes_database: BoolFalse;
  calls_runner: BoolFalse;
  calls_real_provider: BoolFalse;
  uses_network: BoolFalse;
  reads_env: BoolFalse;
  contains_secret: BoolFalse;
  contains_pii: BoolFalse;
  represents_executed_action: BoolFalse;
}

export interface ModelSuggestionReviewDraftSummary {
  kind: 'MODEL_SUGGESTION_REVIEW_DRAFT_SUMMARY';
  version: typeof MODEL_SUGGESTION_REVIEW_DRAFT_GATE_VERSION;
  candidates_built: number;
  review_draft_only: BoolTrue;
  suggestion_only: BoolTrue;
  dry_run_only: BoolTrue;
  executable: BoolFalse;
}

export interface ModelSuggestionReviewDraftTraceSummary {
  kind: 'MODEL_SUGGESTION_REVIEW_DRAFT_TRACE_SUMMARY';
  version: typeof MODEL_SUGGESTION_REVIEW_DRAFT_GATE_VERSION;
  request_id: string;
  source_result_kind: string;
  source_reference_only: BoolTrue;
  validation_checked: BoolTrue;
  candidates_checked: BoolTrue;
  persisted: BoolFalse;
}

export interface ModelSuggestionReviewDraftGateAnswer {
  kind: 'MODEL_SUGGESTION_REVIEW_DRAFT_GATE_ANSWER';
  version: typeof MODEL_SUGGESTION_REVIEW_DRAFT_GATE_VERSION;
  review_draft_gate_blocked: boolean;
  blocked_reason: ModelSuggestionReviewDraftGateBlockedReason | null;
  generated_review_draft_candidates: boolean;
  review_draft_candidates: readonly ModelSuggestionReviewDraftCandidate[];
  review_draft_summary: ModelSuggestionReviewDraftSummary;
  trace_summary: ModelSuggestionReviewDraftTraceSummary;
  emits_review_draft_surface_only: BoolTrue;
  enqueues_review_items: BoolFalse;
  executes_review_items: BoolFalse;
  source_adapter_boundary_result: ModelSuggestionAdapterBoundaryResult;
  contract_only: BoolTrue;
  review_draft_gate_only: BoolTrue;
  review_draft_only: BoolTrue;
  suggestion_only: BoolTrue;
  enters_review_queue: BoolFalse;
  enters_human_confirmation: BoolFalse;
  enters_write_plan: BoolFalse;
  produces_confirmed_action: BoolFalse;
  produces_executable_proposal: BoolFalse;
  emits_confirmed_action_envelope: BoolFalse;
  executes_action: BoolFalse;
  calls_runner: BoolFalse;
  reads_database: BoolFalse;
  writes_database: BoolFalse;
  calls_real_provider: BoolFalse;
  uses_network: BoolFalse;
  reads_env: BoolFalse;
  persists_output: BoolFalse;
}

export interface ModelSuggestionReviewDraftGateResult {
  kind: 'MODEL_SUGGESTION_REVIEW_DRAFT_GATE_RESULT';
  version: typeof MODEL_SUGGESTION_REVIEW_DRAFT_GATE_VERSION;
  plan: ModelSuggestionReviewDraftGatePlan;
  answer: ModelSuggestionReviewDraftGateAnswer;
  persisted: BoolFalse;
  reads_database: BoolFalse;
  writes_database: BoolFalse;
  reads_env: BoolFalse;
  uses_network: BoolFalse;
  calls_real_provider: BoolFalse;
  represents_executed_action: BoolFalse;
  represents_confirmed_action: BoolFalse;
  represents_review_queue_entry: BoolFalse;
  represents_human_confirmation: BoolFalse;
  represents_write_plan: BoolFalse;
  enqueues_review_items: BoolFalse;
  executes_review_items: BoolFalse;
  emits_confirmed_action_envelope: BoolFalse;
}

export interface ModelSuggestionReviewDraftGateValidation {
  ok: boolean;
  blocked_reason: ModelSuggestionReviewDraftGateBlockedReason | null;
}

export function buildModelSuggestionReviewDraftGatePlan(
  request: ModelSuggestionReviewDraftGateRequest,
): ModelSuggestionReviewDraftGatePlan {
  return {
    kind: 'MODEL_SUGGESTION_REVIEW_DRAFT_GATE_PLAN',
    version: MODEL_SUGGESTION_REVIEW_DRAFT_GATE_VERSION,
    executable: FALSE_VALUE,
    persisted: FALSE_VALUE,
    reason: 'model_suggestion_review_draft_gate_readiness_only',
    request,
    allowed_operations: [
      'validate_caller_provided_adapter_boundary_result',
      'project_safe_boundary_candidates_to_review_draft',
      'build_review_draft_gate_result',
    ],
    forbidden_operations: [
      'read_database',
      'write_database',
      'read_runtime_environment',
      'open_transport_channel',
      'perform_live_request',
      'copy_source_output_text',
      'produce_executable_proposal',
      'create_action_entry',
      'create_review_entry',
      'create_human_confirmation_flow',
      'create_write_plan',
      'execute_candidate',
      'use_runtime_runner',
    ],
  };
}

export function projectBoundaryCandidatesToReviewDrafts(
  sourceResult: ModelSuggestionAdapterBoundaryResult,
): readonly ModelSuggestionReviewDraftCandidate[] {
  return sourceResult.answer.boundary_candidates.map((candidate, index) => ({
    kind: 'MODEL_SUGGESTION_REVIEW_DRAFT_CANDIDATE',
    version: MODEL_SUGGESTION_REVIEW_DRAFT_GATE_VERSION,
    review_draft_id: buildReviewDraftId(index),
    source_boundary_candidate_id: candidate.boundary_candidate_id,
    source_suggestion_candidate_id: candidate.source_suggestion_candidate_id,
    source_output_id: candidate.source_output_id,
    review_draft_status: mapReviewDraftStatus(candidate.adaptation_status),
    priority_band: 'medium',
    title: `Review draft surface only ${String(index + 1).padStart(3, '0')}`,
    summary: 'Safe boundary metadata was projected to a draft-only review surface and was not recorded for downstream handling.',
    evidence_summary: summarizeCount(candidate.evidence_refs.length, 'evidence reference'),
    risk_summary: summarizeCount(candidate.risk_flags.length, 'risk flag'),
    review_draft_only: TRUE_VALUE,
    suggestion_only: TRUE_VALUE,
    dry_run_only: TRUE_VALUE,
    requires_human_review: TRUE_VALUE,
    enqueued: FALSE_VALUE,
    executable: FALSE_VALUE,
    confirmed_action: FALSE_VALUE,
    confirmed_action_envelope: FALSE_VALUE,
    human_confirmed: FALSE_VALUE,
    approval_recorded: FALSE_VALUE,
    enters_review_queue: FALSE_VALUE,
    enters_human_confirmation: FALSE_VALUE,
    enters_write_plan: FALSE_VALUE,
    emits_confirmed_action_envelope: FALSE_VALUE,
    emits_review_queue_candidate: FALSE_VALUE,
    produces_confirmed_action: FALSE_VALUE,
    produces_executable_proposal: FALSE_VALUE,
    reads_database: FALSE_VALUE,
    writes_database: FALSE_VALUE,
    calls_runner: FALSE_VALUE,
    calls_real_provider: FALSE_VALUE,
    uses_network: FALSE_VALUE,
    reads_env: FALSE_VALUE,
    contains_secret: FALSE_VALUE,
    contains_pii: FALSE_VALUE,
    represents_executed_action: FALSE_VALUE,
  }));
}

export function runModelSuggestionReviewDraftGate(
  plan: ModelSuggestionReviewDraftGatePlan,
): ModelSuggestionReviewDraftGateResult {
  const requestValidation = validateModelSuggestionReviewDraftGateRequest(plan.request);
  if (!requestValidation.ok) return buildReviewDraftGateResult(plan, [], requestValidation.blocked_reason);

  const sourceResult = plan.request.source_adapter_boundary_result;
  const sourceValidation = validateSourceAdapterBoundaryResult(sourceResult);
  if (!sourceValidation.ok) return buildReviewDraftGateResult(plan, [], sourceValidation.blocked_reason);

  const candidates = projectBoundaryCandidatesToReviewDrafts(sourceResult);
  for (const candidate of candidates) {
    const candidateValidation = validateModelSuggestionReviewDraftCandidate(candidate, sourceResult);
    if (!candidateValidation.ok) return buildReviewDraftGateResult(plan, [], candidateValidation.blocked_reason);
  }

  const result = buildReviewDraftGateResult(plan, candidates, null);
  const resultValidation = validateModelSuggestionReviewDraftGateResult(result);
  if (!resultValidation.ok) return buildReviewDraftGateResult(plan, [], resultValidation.blocked_reason);
  return result;
}

export function validateModelSuggestionReviewDraftGateRequest(
  request: unknown,
): ModelSuggestionReviewDraftGateValidation {
  const record = asRecord(request);
  if (record?.kind !== 'MODEL_SUGGESTION_REVIEW_DRAFT_GATE_REQUEST') return blocked('invalid_request_kind');
  if (record.allow_enqueue === true) return blocked('illegal_enqueue_allowed');
  if (record.allow_review_queue_entry === true) return blocked('illegal_review_queue_entry_allowed');
  if (record.allow_confirmed_action === true) return blocked('illegal_confirmed_action_allowed');
  if (record.allow_confirmed_action_envelope === true) return blocked('illegal_confirmed_action_envelope_allowed');
  if (record.allow_human_confirmation === true) return blocked('illegal_human_confirmation_allowed');
  if (record.allow_runner === true) return blocked('illegal_runner_allowed');
  if (record.allow_execution === true) return blocked('illegal_execution_allowed');
  if (record.allow_write_plan_entry === true) return blocked('illegal_write_plan_entry_allowed');
  if (record.allow_db === true) return blocked('illegal_db_allowed');
  if (record.allow_network === true) return blocked('illegal_network_allowed');
  if (record.allow_env_read === true) return blocked('illegal_env_read_allowed');
  if (record.allow_model_call === true) return blocked('illegal_model_call_allowed');
  return { ok: true, blocked_reason: null };
}

export function validateSourceAdapterBoundaryResult(
  result: unknown,
): ModelSuggestionReviewDraftGateValidation {
  const record = asRecord(result);
  if (record?.kind !== 'MODEL_SUGGESTION_ADAPTER_BOUNDARY_RESULT') return blocked('invalid_source_result_kind');
  if (record.calls_real_provider === true) return blocked('illegal_source_called_real_provider');
  if (record.uses_network === true) return blocked('illegal_source_used_network');
  if (record.reads_database === true) return blocked('illegal_source_reads_database');
  if (record.writes_database === true) return blocked('illegal_source_writes_database');

  const answer = asRecord(record.answer);
  if (answer === null) return blocked('source_answer_missing');
  if (answer.adapter_boundary_blocked === true) return blocked('source_adapter_boundary_blocked');
  if (answer.generated_boundary_candidates === false) return blocked('source_boundary_candidates_missing');
  if (answer.calls_real_provider === true) return blocked('illegal_source_called_real_provider');
  if (answer.uses_network === true) return blocked('illegal_source_used_network');
  if (answer.reads_database === true) return blocked('illegal_source_reads_database');
  if (answer.writes_database === true) return blocked('illegal_source_writes_database');
  if (answer.enters_review_queue === true) return blocked('illegal_source_entered_review_queue');
  if (answer.enters_human_confirmation === true) return blocked('illegal_source_entered_human_confirmation');
  if (answer.enters_write_plan === true) return blocked('illegal_source_entered_write_plan');
  if (answer.produces_confirmed_action === true) return blocked('illegal_source_produced_confirmed_action');
  if (answer.produces_executable_proposal === true) {
    return blocked('illegal_source_produced_executable_proposal');
  }
  if (!('boundary_candidates' in answer)) return blocked('source_boundary_candidates_missing');
  if (!Array.isArray(answer.boundary_candidates)) return blocked('source_boundary_candidates_missing');
  if (answer.boundary_candidates.length === 0) return blocked('source_boundary_candidates_empty');

  for (const candidate of answer.boundary_candidates) {
    const candidateValidation = validateSourceBoundaryCandidate(candidate, result as ModelSuggestionAdapterBoundaryResult);
    if (!candidateValidation.ok) return candidateValidation;
  }

  return { ok: true, blocked_reason: null };
}

export function validateSourceBoundaryCandidate(
  candidate: unknown,
  sourceResult?: ModelSuggestionAdapterBoundaryResult,
): ModelSuggestionReviewDraftGateValidation {
  const record = asRecord(candidate);
  const status = typeof record?.adaptation_status === 'string' ? record.adaptation_status : '';
  if (!ALLOWED_SOURCE_ADAPTATION_STATUS_VALUES.has(status)) return blocked('illegal_source_candidate_status');
  if (DANGEROUS_FORWARD_STATUS_VALUES.has(status)) return blocked('illegal_source_candidate_status');
  if (record?.executable === true) return blocked('illegal_source_candidate_executable');
  if (record?.confirmed_action === true) return blocked('illegal_source_candidate_confirmed_action');
  if (record?.human_confirmed === true) return blocked('illegal_source_candidate_human_confirmed');
  if (record?.emits_confirmed_action_envelope === true) {
    return blocked('illegal_source_candidate_emits_confirmed_action_envelope');
  }
  if (record?.enters_review_queue === true) return blocked('illegal_source_candidate_enters_review_queue');
  if (record?.enters_write_plan === true) return blocked('illegal_source_candidate_enters_write_plan');
  if ('output_text' in (record ?? {})) return blocked('illegal_output_text_propagated');
  if (containsSourceOutputText(record, sourceResult)) return blocked('illegal_output_text_propagated');
  return { ok: true, blocked_reason: null };
}

export function validateModelSuggestionReviewDraftCandidate(
  candidate: unknown,
  sourceResult?: ModelSuggestionAdapterBoundaryResult,
): ModelSuggestionReviewDraftGateValidation {
  const record = asRecord(candidate);
  const status = typeof record?.review_draft_status === 'string' ? record.review_draft_status : '';
  if (!ALLOWED_REVIEW_DRAFT_STATUS_VALUES.has(status)) return blocked('illegal_source_candidate_status');
  if (DANGEROUS_FORWARD_STATUS_VALUES.has(status)) return blocked('illegal_source_candidate_status');
  if (record?.enqueued === true) return blocked('illegal_draft_candidate_enqueued');
  if (record?.executable === true) return blocked('illegal_draft_candidate_executable');
  if (record?.confirmed_action === true) return blocked('illegal_draft_candidate_confirmed_action');
  if (record?.confirmed_action_envelope === true) {
    return blocked('illegal_draft_candidate_confirmed_action_envelope');
  }
  if (record?.emits_confirmed_action_envelope === true) {
    return blocked('illegal_draft_candidate_emits_confirmed_action_envelope');
  }
  if (record?.emits_review_queue_candidate === true) {
    return blocked('illegal_draft_candidate_emits_review_queue_candidate');
  }
  if (record?.enters_review_queue === true) return blocked('illegal_draft_candidate_enters_review_queue');
  if (record?.enters_write_plan === true) return blocked('illegal_draft_candidate_enters_write_plan');
  if ('output_text' in (record ?? {})) return blocked('illegal_output_text_propagated');
  if (containsSourceOutputText(record, sourceResult)) return blocked('illegal_output_text_propagated');
  return { ok: true, blocked_reason: null };
}

export function validateModelSuggestionReviewDraftGateResult(
  result: unknown,
): ModelSuggestionReviewDraftGateValidation {
  const record = asRecord(result);
  const answer = asRecord(record?.answer);
  if (record?.persisted === true || answer?.persists_output === true) return blocked('illegal_execution_allowed');
  if (record?.enqueues_review_items === true || answer?.enqueues_review_items === true) {
    return blocked('illegal_draft_candidate_enqueued');
  }
  if (record?.executes_review_items === true || answer?.executes_review_items === true) {
    return blocked('illegal_execution_allowed');
  }
  if (record?.emits_confirmed_action_envelope === true || answer?.emits_confirmed_action_envelope === true) {
    return blocked('illegal_draft_candidate_emits_confirmed_action_envelope');
  }
  if (record?.calls_real_provider === true || answer?.calls_real_provider === true) {
    return blocked('illegal_source_called_real_provider');
  }
  if (record?.uses_network === true || answer?.uses_network === true) return blocked('illegal_source_used_network');
  if (record?.reads_env === true || answer?.reads_env === true) return blocked('illegal_env_read_allowed');
  if (record?.reads_database === true || answer?.reads_database === true) {
    return blocked('illegal_source_reads_database');
  }
  if (record?.writes_database === true || answer?.writes_database === true) {
    return blocked('illegal_source_writes_database');
  }
  if (record?.represents_executed_action === true) return blocked('illegal_execution_allowed');
  if (record?.represents_confirmed_action === true) return blocked('illegal_source_produced_confirmed_action');
  if (record?.represents_review_queue_entry === true) return blocked('illegal_source_entered_review_queue');
  if (record?.represents_human_confirmation === true) {
    return blocked('illegal_source_entered_human_confirmation');
  }
  if (record?.represents_write_plan === true) return blocked('illegal_source_entered_write_plan');
  if (answer?.produces_executable_proposal === true) {
    return blocked('illegal_source_produced_executable_proposal');
  }
  if (answer?.produces_confirmed_action === true) return blocked('illegal_source_produced_confirmed_action');
  if (answer?.enters_review_queue === true) return blocked('illegal_source_entered_review_queue');
  if (answer?.enters_write_plan === true) return blocked('illegal_source_entered_write_plan');

  const sourceResult = answer?.source_adapter_boundary_result as ModelSuggestionAdapterBoundaryResult | undefined;
  const candidates = Array.isArray(answer?.review_draft_candidates) ? answer.review_draft_candidates : [];
  for (const candidate of candidates) {
    const validation = validateModelSuggestionReviewDraftCandidate(candidate, sourceResult);
    if (!validation.ok) return validation;
  }
  return { ok: true, blocked_reason: null };
}

function buildReviewDraftGateResult(
  plan: ModelSuggestionReviewDraftGatePlan,
  candidates: readonly ModelSuggestionReviewDraftCandidate[],
  blockedReason: ModelSuggestionReviewDraftGateBlockedReason | null,
): ModelSuggestionReviewDraftGateResult {
  const gateBlocked = blockedReason !== null;
  const visibleCandidates = gateBlocked ? [] : candidates;
  const sourceResult = plan.request.source_adapter_boundary_result;
  return {
    kind: 'MODEL_SUGGESTION_REVIEW_DRAFT_GATE_RESULT',
    version: MODEL_SUGGESTION_REVIEW_DRAFT_GATE_VERSION,
    plan,
    answer: {
      kind: 'MODEL_SUGGESTION_REVIEW_DRAFT_GATE_ANSWER',
      version: MODEL_SUGGESTION_REVIEW_DRAFT_GATE_VERSION,
      review_draft_gate_blocked: gateBlocked,
      blocked_reason: blockedReason,
      generated_review_draft_candidates: !gateBlocked && visibleCandidates.length > 0,
      review_draft_candidates: visibleCandidates,
      review_draft_summary: {
        kind: 'MODEL_SUGGESTION_REVIEW_DRAFT_SUMMARY',
        version: MODEL_SUGGESTION_REVIEW_DRAFT_GATE_VERSION,
        candidates_built: visibleCandidates.length,
        review_draft_only: TRUE_VALUE,
        suggestion_only: TRUE_VALUE,
        dry_run_only: TRUE_VALUE,
        executable: FALSE_VALUE,
      },
      trace_summary: {
        kind: 'MODEL_SUGGESTION_REVIEW_DRAFT_TRACE_SUMMARY',
        version: MODEL_SUGGESTION_REVIEW_DRAFT_GATE_VERSION,
        request_id: plan.request.request_id,
        source_result_kind: String(sourceResult?.kind ?? ''),
        source_reference_only: TRUE_VALUE,
        validation_checked: TRUE_VALUE,
        candidates_checked: TRUE_VALUE,
        persisted: FALSE_VALUE,
      },
      emits_review_draft_surface_only: TRUE_VALUE,
      enqueues_review_items: FALSE_VALUE,
      executes_review_items: FALSE_VALUE,
      source_adapter_boundary_result: sourceResult,
      contract_only: TRUE_VALUE,
      review_draft_gate_only: TRUE_VALUE,
      review_draft_only: TRUE_VALUE,
      suggestion_only: TRUE_VALUE,
      enters_review_queue: FALSE_VALUE,
      enters_human_confirmation: FALSE_VALUE,
      enters_write_plan: FALSE_VALUE,
      produces_confirmed_action: FALSE_VALUE,
      produces_executable_proposal: FALSE_VALUE,
      emits_confirmed_action_envelope: FALSE_VALUE,
      executes_action: FALSE_VALUE,
      calls_runner: FALSE_VALUE,
      reads_database: FALSE_VALUE,
      writes_database: FALSE_VALUE,
      calls_real_provider: FALSE_VALUE,
      uses_network: FALSE_VALUE,
      reads_env: FALSE_VALUE,
      persists_output: FALSE_VALUE,
    },
    persisted: FALSE_VALUE,
    reads_database: FALSE_VALUE,
    writes_database: FALSE_VALUE,
    reads_env: FALSE_VALUE,
    uses_network: FALSE_VALUE,
    calls_real_provider: FALSE_VALUE,
    represents_executed_action: FALSE_VALUE,
    represents_confirmed_action: FALSE_VALUE,
    represents_review_queue_entry: FALSE_VALUE,
    represents_human_confirmation: FALSE_VALUE,
    represents_write_plan: FALSE_VALUE,
    enqueues_review_items: FALSE_VALUE,
    executes_review_items: FALSE_VALUE,
    emits_confirmed_action_envelope: FALSE_VALUE,
  };
}

function buildReviewDraftId(index: number): string {
  return `MODEL_SUGGESTION_REVIEW_DRAFT_${String(index + 1).padStart(3, '0')}`;
}

function mapReviewDraftStatus(
  adaptationStatus: AdaptedModelSuggestionBoundaryCandidate['adaptation_status'],
): ModelSuggestionReviewDraftStatus {
  if (adaptationStatus === 'boundary_policy_only') return 'draft_policy_only';
  if (adaptationStatus === 'boundary_blocked_source') return 'draft_blocked_source';
  return 'draft_requires_human_review';
}

function summarizeCount(count: number, label: string): string {
  return `${count} ${label}${count === 1 ? '' : 's'} carried as safe summary metadata.`;
}

function containsSourceOutputText(
  candidate: Record<string, unknown> | null,
  sourceResult: ModelSuggestionAdapterBoundaryResult | undefined,
): boolean {
  const sourceText = sourceResult
    ?.answer
    ?.source_suggest_only_output_gate_result
    ?.answer
    ?.source_model_output_envelope
    ?.output_text;
  if (!sourceText || candidate === null) return false;
  const title = typeof candidate.title === 'string' ? candidate.title : '';
  const summary = typeof candidate.summary === 'string' ? candidate.summary : '';
  const evidenceSummary = typeof candidate.evidence_summary === 'string' ? candidate.evidence_summary : '';
  const riskSummary = typeof candidate.risk_summary === 'string' ? candidate.risk_summary : '';
  return [title, summary, evidenceSummary, riskSummary].some(value => value === sourceText || value.includes(sourceText));
}

function blocked(
  reason: ModelSuggestionReviewDraftGateBlockedReason,
): ModelSuggestionReviewDraftGateValidation {
  return { ok: false, blocked_reason: reason };
}

function asRecord(value: unknown): Record<string, unknown> | null {
  if (value === null || typeof value !== 'object') return null;
  return value as Record<string, unknown>;
}

const ALLOWED_SOURCE_ADAPTATION_STATUS_VALUES = new Set([
  'boundary_requires_human_review',
  'boundary_blocked_source',
  'boundary_policy_only',
]);

const ALLOWED_REVIEW_DRAFT_STATUS_VALUES = new Set([
  'draft_requires_human_review',
  'draft_blocked_source',
  'draft_policy_only',
]);

const DANGEROUS_FORWARD_STATUS_VALUES = new Set([
  'ready',
  'approved',
  'confirmed',
  'executable',
  'queued',
  'sent_to_review',
  'runnable',
  'success',
  'completed',
  'written',
  'enqueued',
]);
