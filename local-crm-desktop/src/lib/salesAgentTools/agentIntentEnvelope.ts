...
export function createAgentIntentEnvelope(
  ...
  // They require the user to choose one bounded action before any write/search route.
  const boundedNamedSwitchContinuation = search.is_explicit_switch
    && Boolean(search.filters.name_query)
    && !/(?:另一家|别家|其他客户)/.test(intentText)
    && /(?:后|然后|再|之后).*(?:总结|概括|风险|分析|比较|排序|下一步)/.test(intentText);
  if (hasCompetingActions(intentText) && !boundedNamedSwitchContinuation) {
    return make({
...
function hasCustomerSummaryMeaning(text: string): boolean {
  return /总结|概括|概述|汇总|摘要|简介|归拢.*资料|压缩.*信息|全貌|全景|速览|总览|概况|概览|画像|简述|brief|overview|customer\s*summary|整体(?:怎么样|情况|面)|目前(?:的)?情况|最近的情况|基本情况|基本盘|底子|来龙去脉|当前(?:局面|进展)|现阶段的信息|是什么情形|现况|合作进展|客户现状|客户状态|客户背景|客户信息|这个客户|这位客户|这客户|这家公司|这家企业|这家合作方|这个账户|这单客户|刚接手|新接手|客户侧现状|客户咋样|客户怎么样|客怎么样|现在啥(?:情况|进度)|瞅瞅.*客户/i.test(text);
}
