...
export function createApprovedCrmWriteBoundary(repository: ApprovedCrmWriteRepository): SafeWriteBoundary {
  return {
    async execute(proposal: AgentWriteProposal, _confirmationId: string) {
      const now = new Date().toISOString(); const values = proposal.proposed_values;
