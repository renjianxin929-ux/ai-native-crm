...
export function createSalesAgentDraft(kind: DraftKind, interaction: SalesAgentInteractionResult) { return { kind, content: interaction.runtime.result.customer_summary.value, evidence_refs: interaction.evidence_refs, requires_human_review: true as const, sent: false as const, written: false as const }; }
export function proposeConfirmedCrmAction(interaction: SalesAgentInteractionResult) { return { proposal_only: true as const, requires_human_confirmation: true as const, executable: false as const, writes_crm: false as const, evidence_refs: interaction.evidence_refs }; }
