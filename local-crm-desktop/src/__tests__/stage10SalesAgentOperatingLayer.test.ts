...
import { intentFromEnvelope, createSalesAgentDraft, proposeConfirmedCrmAction, proposeSalesAgentPlan, runSalesAgentInteraction, validateSalesAgentPlan } from '../lib/salesAgentTools/operatingLayer';
import { createAgentIntentEnvelope } from '../lib/salesAgentTools/agentIntentEnvelope';
const env = (message: string) => createAgentIntentEnvelope(message, '2026-07-12T00:00:00.000Z');
...
describe('Stage10.5 Sales Agent intelligence polish', () => {
  ...
  it('rejects unknown tools and over-limit plans', () => { const plan = proposeSalesAgentPlan(env('Prepare me'), 'customer-1'); expect(() => validateSalesAgentPlan({ ...plan, steps: [{ tool_id: 'unknown' as never, customer_id: 'customer-1' }] }, 'customer-1')).toThrow('unknown tool'); expect(() => validateSalesAgentPlan({ ...plan, steps: Array.from({ length: 6 }, () => plan.steps[0]) }, 'customer-1')).toThrow('one to five'); });
  it('retires the duplicate legacy runtime instead of preserving a competing model path', async () => { await expect(runSalesAgentInteraction('准备跟进文案', { customer_id: 'customer-1', snapshot, context, memory, profile_id: 'foreign_trade_geo' })).rejects.toThrow(/retired/); expect(createSalesAgentDraft).toBeTypeOf('function'); expect(proposeConfirmedCrmAction).toBeTypeOf('function'); });
  it('never infers an unregistered intent or direct provider call', () => { expect(intentFromEnvelope(env(''))).toBe('SAFE_FALLBACK'); expect(() => proposeSalesAgentPlan(env(''), 'customer-1')).toThrow('user message'); });
});
