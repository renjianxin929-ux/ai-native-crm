...
describe('T21 — ORIGINAL 20 FOUNDATION UNCHANGED + W4-1 customer.create + W4-2 customer.profile.update + W4-4 customer.delete + W4-3 visit.create + C0 customer.opportunity_amount.update: registry=25, bindings=25, unbound=0', () => {
  it('production registry/bindings remain exactly 25 with zero unbound', () => {
...
describe('T29 — WAVE-4+C0 IDENTITIES: customer.create + customer.profile.update + customer.delete + visit.create + customer.opportunity_amount.update registered; visit.update / visit.delete / import.execute remain absent (count=25)', () => {
  it('registry contains exactly the 25 frozen capabilities; create / profile.update / delete / visit.create are the Wave-4 identities and opportunity_amount.update is the C0 identity', () => {
