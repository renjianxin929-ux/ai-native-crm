...
describe('SalesAgentSession production read/runtime and reviewed-capture integration', () => {
  it('executes registered reads, passes ACTIVE memory to runtime, and returns execution-derived trace/evidence', async () => {
    ...
    const session = new SalesAgentSession('customer-1', { reason, capture: async () => ({ visual_facts: [] }) }, () => '2026-07-12T00:00:00.000Z', { snapshot, context, memory, profile_id: 'foreign_trade_geo' });
    const outcome = await session.submit('Summarize the customer using evidence.');
    ...
    expect(outcome.result.provider).toContain('SalesAgentRuntime');
  ...
  it('keeps capture pending/rejected facts out, sends accepted edited content through explicit reasoning only, and preserves source reference', async () => {
    ...
    const session = new SalesAgentSession('customer-1', { reason, capture: async () => ({ visual_facts: [{ fact_id: 'accept', fact_type: 'visible_requirement', content: 'Original accepted content', source_reference: 'image:1', confidence: .8 }, { fact_id: 'reject', fact_type: 'visible_requirement', content: 'Rejected content', source_reference: 'image:2', confidence: .8 }] }) }, () => '2026-07-12T00:00:00.000Z', { snapshot, context, memory, profile_id: 'foreign_trade_geo' });
